<?php
    ...
    foreach($decoded as $arrayCity) {
        ...
        $arrayCityTemp += array($cityNameSearched =>  $latitudeCity.','.$longitudeCity );        
        
    }
    
 
    $arrayDistinctCityValue = array_unique($arrayCityTemp);

    
    foreach($arrayDistinctCityValue as $cityVal => $latLong) {
        $cityNameSearched = $cityVal;
        $arr = explode(",", $latLong, 2);
        $latitudeCity = $arr[0];        
        $longitudeCity = $arr[1];  
